...
import { createEdge, shouldCreateConnection } from './connectionRules'
...
function parseItems(lines: string[]): { regularItems: ParsedItem[], ellipseItems: ParsedItem[], requirements: Requirement[] } {
  ...
  lines.forEach(line => {
    ...
    if (line.startsWith('//')) {
      const [category, content] = line.slice(2).split(':').map(s => s.trim())
      if (['P', 'S', 'A', 'U'].includes(category)) {
        requirements.push({ category: category as 'P' | 'S' | 'A' | 'U', content })
      }
      return
    }
...
export function parseInput(input: string, showAllRequirements: boolean): ParseResult {
  ...
  let nodeId = 0
  let ellipseNodeId = 0
  let pendingConnections: Array<{ source: string; target: string; sourceHandle: string; label?: string }> = []
  ...
  function processNode(screen: string, items: string[]) {
    ...
    regularItems.forEach((item, itemIndex) => {
      if (item.connection) {
        const targetNodeIndex = nodes.findIndex(n => n.data.label === item.connection)
  ...
  pendingConnections.forEach(connection => {
    const targetNodeIndex = nodes.findIndex(n => n.data.label === connection.target)
    if (targetNodeIndex !== -1) {
      edges.push(createEdge(
        connection.source,
        nodes[targetNodeIndex].id,
