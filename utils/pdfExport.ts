...
export async function exportToPDF(flowRef: React.RefObject<HTMLDivElement>, fitBounds: (bounds: any) => void, getNodes: () => any[], getViewport: () => any, fitView: (options: any) => void) {
  if (!flowRef.current) {
  ...
  try {
    ...
    const controlsElement = flowRef.current.querySelector('.react-flow__controls')
    ...
    const nodesBounds = nodes.reduce(
      (acc, node) => {
        ...
        if (node.position.x + (node.width || 0) > acc.maxX) acc.maxX = node.position.x + (node.width || 0)
        if (node.position.y + (node.height || 0) > acc.maxY) acc.maxY = node.position.y + (node.height || 0)
        return acc
      },
