...
export function useScreenFlow() {
  ...
  const handleGenerate = useCallback(() => {
    const { nodes: newNodes, edges: newEdges } = parseInput(input, showAllRequirements)
    setNodes(newNodes)
    setEdges(newEdges)
  ...
  const onConnect = useCallback((params: Connection) => {
    ...
    if (sourceNode && targetNode && shouldCreateConnection(sourceNode, targetNode)) {
      setEdges((eds) => addEdge(createEdge(params.source!, params.target!, params.sourceHandle!), eds))
    }
  }, [nodes, setEdges])
